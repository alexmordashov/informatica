...
int myBinSearch(vector<double> &mas, int &r, double x, string start) {
    int position = start.find(".");
    string after_dot = start.substr(position + 1);
    int L = 0, R = mas.size() - 1, q;
    while (L <= R) {
        q = (R + L) / 2;
        double razn = abs(mas[q] - x);
        //cout << razn << " " << pow(0.1, after_dot.size()) << " " << after_dot.size() << endl;
        if (razn < pow(0.1, after_dot.size()))
            return q;
        else if (mas[q] < x)
            L = q + 1;
        else if (mas[q] > x)
